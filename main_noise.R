...
preprocessed_data <- preprocessed_results_by_sample[["sd"]]
...
# for (percentage in zero_percentages) { #percentage =30
  ...
  write.csv(results_df, file = paste0("noise_", ".csv"), row.names = FALSE)
  
  print(paste("Results for", "% zero values have been saved"))
...
# Zero percentage settings i =2 percentage =30
zero_percentages <- c(30)
...
for (percentage in zero_percentages) {
  ...
  print(results_df)
  # Save the data frame to a CSV file
  write.csv(results_df, file = paste0("noise_noMe_", percentage, ".csv"), row.names = FALSE)
...
generate_power_results <- function(file_name, zero_percentage) {
  ...
  results_df$noise_size <- rep(c(0.5, 1, 1.5, 2, 2.5, 3, 3.5, 4, 4.5, 5), each = 100)
...
power_results_30 <- generate_power_results("noise_.csv", "30% Zero Values")
...
generate_power_results <- function(file_name, zero_percentage) {
  ...
  results_df$sample_size <- rep(c(0.5, 1, 1.5, 2, 2.5, 3, 3.5, 4, 4.5, 5), each = 100)
...
# Generate Power results for each sample
power_results_low <- generate_power_results("noise_noMe_30.csv", "low")

# Combine all Power results
all_power_results <- power_results_low

# Plot Power analysis with line chart
ggplot(all_power_results, aes(x = sample_size, y = power, color = Method, group = Method)) +
  geom_line(size = 0.8) +  # Make lines thinner
  geom_point(size = 1.5) +  # Smaller points
  theme_minimal() +
  ...
  coord_cartesian(ylim = c(0, 1)) +  # Fixed y-axis range
  facet_grid(. ~ Zero_Percentage)
ggsave(filename = "10-10-pic/noise_1_line.png", plot = last_plot(), width = 10, height = 8)
