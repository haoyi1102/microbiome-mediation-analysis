...
for (percentage in zero_percentages) { #percentage = 100
  results <- list()
  ...
  write.csv(results_df, file = paste0("10_1_new_feature_test_", percentage, ".csv"), row.names = FALSE)
...
# Generate power results for each sample with specified order of Zero_Percentage
power_results_10 <- generate_power_results("10_1_new_feature_test_1.csv", "Low")
power_results_30 <- generate_power_results("10_1_new_feature_test_1000.csv", "Medium")
power_results_50 <- generate_power_results("10_1_new_feature_test_100.csv", "High")

# Combine all power results
all_power_results <- bind_rows(power_results_10, power_results_30, power_results_50)

# Set the order of the Zero_Percentage factor
all_power_results$Zero_Percentage <- factor(all_power_results$Zero_Percentage, levels = c("Low", "Medium", "High"))

# Plot power analysis with facet_grid using the correct order for Zero Percentage
...
ggsave(filename = "10-10-pic/feature_1_hist.png", plot = last_plot(), width = 10, height = 8)
...
# 保存图片到新文件夹
ggsave(filename = "10-10-pic/feature_1_line.png", plot = last_plot(), width = 10, height = 8)
