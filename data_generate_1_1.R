...
# BiocManager::install("SparseDOSSA2") devtools::install_github("biobakery/SparseDOSSA2")
# ?control_fit
# ?SparseDOSSA2 template <-  readRDS("sparsedossa_baseline.rds")
...
generate_simulation_data <- function(n_sample = 100, template = template, n_feature = 20, 
                                     ...
                                     median_read_depth = 100000, alpha_0 = 1, alpha_T = 0.5, 
  ...
  # 
  # # 生成随机顺序的二元变量
  # metadata$treatment_binary <- sample(ifelse(metadata$treatment == "Control", 0, 1))
  ...
  for (i in 1:nrow(feature_metadata_spike_df)) {
    ...
    } else if (feature_metadata_spike_df$effect_size[i] < 0) {
      alpha_M[feature_index] <- -alpha_M_value  
    }
