...
apply_bernoulli_zero_generation <- function(M_matrix) {
  # 计算概率矩阵，概率与计数值成反比
  prob_matrix <- 1 / (M_matrix + 1)
  
  # 初始化一个矩阵用于存储结果
  zero_matrix <- matrix(0, nrow = nrow(M_matrix), ncol = ncol(M_matrix))
  
  # 使用伯努利试验生成零值
  for (i in 1:nrow(M_matrix)) {
    for (j in 1:ncol(M_matrix)) {
      zero_matrix[i, j] <- ifelse(runif(1) < prob_matrix[i, j], 0, M_matrix[i, j])
    }
  }
...
for (percentage in zero_percentages) { # percentage = 10
  results <- list()
  ...
  for (i in 1:length(preprocessed_data)) { # i=1
    ...
    # Set zero percentage
    M_matrix <- subsample_and_generate_zeros(M_matrix, target_zero_proportion  = 70)
    
    subsample_and_generate_zeros <- function(count_matrix, target_depth = NULL, target_zero_proportion ) {
      ...
      for (i in 1:nrow(count_matrix)) {
        ...
        if (additional_zeros_needed > 0) {
          ...
          probabilities <- probabilities / sum(probabilities)  # 正规化概率
  ...
  write.csv(results_df, file = paste0("sample_", percentage, ".csv"), row.names = FALSE)
