...
zero_percentages <- c(10000,500,1)

for (percentage in zero_percentages) { #percentage =40
  results <- list()
  ...
  for (i in 1:length(preprocessed_data)) { #i=5
    ...
    M_a_matrix <- set_zero_percentage_new(count_matrix=M_a_matrix, target_zero_proportion=percentage)
    M_matrix <- M_a_matrix / rowSums(M_a_matrix)
  ...
  write.csv(results_df, file = paste0("sample_prob_", percentage, ".csv"), row.names = FALSE)
...
power_results_30 <- generate_power_results("sample_prob_10000.csv", "low")
power_results_50 <- generate_power_results("sample_prob_500.csv", "medium")
power_results_70 <- generate_power_results("sample_prob_1.csv", "high")

# Combine all power results
all_power_results <- bind_rows(power_results_70, power_results_30, power_results_50 )
